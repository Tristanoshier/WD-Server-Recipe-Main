const APIKEY_ERROR = 'API KEY ERROR';
const CALORIE_SUCCESS = 'Calorie Estimate Success';
const CREATE_SUCCESS = 'Create Success';
const EMAIL_ALREADY_IN_USE = 'This email is already in use!';
const FAILED_TO_REGISTER_CLIENT = 'Failed to register client';
const GET_SUCCESS = 'Get Request Successful';
const GROUP_API_NOT_FOUND = 'Group API Record Not Found';
const ID_DOES_NOT_EXIST = ':id does not exist';
const INCORRECT_EMAIL_PASSWORD = 'Incorrect Email and Password combination!';
const INVALID_DATE = 'Invalid Date Format: MM-DD-YYYY';
const INVALID_TOKEN = 'Invalid Token';
const RECIPE_CREATE_ERROR = "Recipe Create Error";
const MISSING_API_KEY = 'Missing API KEY';
const MISSING_KEY_VALUE_PAIRS = 'Missing Key: Value Pairs';
const NAN_ERROR = 'ID is Not a number';
const PASSWORDS_DO_NOT_MATCH = 'Passwords do not Match';
const PROFILE_ALREADY_EXISTS = 'Profile already exists';
const PROFILE_ERROR = 'Profile Endpoint Failed';
const TITLE_DELETE_SUCCESS = 'Delete ran successfully';
const TITLE_LOGIN_ERROR = 'Login Error';
const TITLE_RECIPE_ERROR = 'Recipe Endpoint Error';
const TITLE_SIGNUP_ERROR = 'Signup Error';
const TITLE_UPDATE_SUCCESS = 'Update Ran Successfully';
const TOKEN_ERROR = 'Token Error';
const USER_CREATED = 'User created must login to get token';
const USER_FOUND = 'User Found';
module.exports = {
  APIKEY_ERROR,
  CALORIE_SUCCESS,
  CREATE_SUCCESS,
  EMAIL_ALREADY_IN_USE,
  FAILED_TO_REGISTER_CLIENT,
  GET_SUCCESS,
  GROUP_API_NOT_FOUND,
  ID_DOES_NOT_EXIST,
  INCORRECT_EMAIL_PASSWORD,
  INVALID_DATE,
  INVALID_TOKEN,
  MISSING_KEY_VALUE_PAIRS,
  MISSING_API_KEY,
  NAN_ERROR,
  PASSWORDS_DO_NOT_MATCH,
  PROFILE_ALREADY_EXISTS,
  PROFILE_ERROR,
  TITLE_DELETE_SUCCESS,
  TITLE_LOGIN_ERROR,
  TITLE_SIGNUP_ERROR,
  TITLE_UPDATE_SUCCESS,
  TOKEN_ERROR,
  USER_CREATED,
  USER_FOUND,
  RECIPE_CREATE_ERROR,
  TITLE_RECIPE_ERROR
};
